...
def api(monkeypatch):
    ...
    def install(*responses):
        ...
        monkeypatch.setattr(hdb_resale.requests, "get", fake_get)
        return calls
...
def test_a_good_page_comes_back_as_the_result_block(api):
    ...
    assert hdb_resale.request_chunk(0)["records"] == [{"_id": 1, "month": "2024-01"}]
...
def test_success_false_is_an_error_not_a_payload(api):
    ...
    with pytest.raises(hdb_resale.IngestError, match="success=false"):
        hdb_resale.request_chunk(0)
...
def test_client_errors_fail_on_the_first_attempt(api, status):
    ...
    with pytest.raises(hdb_resale.IngestError, match="not retryable"):
        hdb_resale.request_chunk(0)
...
def test_rate_limiting_is_retried_and_recovers(api):
    ...
    assert hdb_resale.request_chunk(0)["records"] == [{"_id": 1}]
    assert len(calls) == 2
...
def test_rate_limiting_gives_up_eventually(api):
    ...
    with pytest.raises(hdb_resale.IngestError, match="rate-limited"):
        hdb_resale.request_chunk(0)

    assert len(calls) == hdb_resale.MAX_RETRIES
...
def test_server_errors_are_still_retried(api):
    ...
    with pytest.raises(hdb_resale.IngestError, match="failed after"):
        hdb_resale.request_chunk(0)
...
def test_a_dropped_connection_is_retried_then_recovers(monkeypatch):
    ...
    monkeypatch.setattr(hdb_resale.requests, "get", flaky_get)
