...
The descriptive figures exist twice -- in SQL, as marts, and in pandas, in dashboard/explore.py,
...
1 on any difference beyond the mart's own rounding.
    ...
    python -m dashboard.check_parity                                 # the committed edition
    python -m dashboard.check_parity --edition data/fixture/published   # CI
"""
...
def compare(mart, computed, key, tolerances):
    ...
    for missing in sorted(set(published.index) ^ set(recomputed.index), key=str):
        side = "the mart" if missing in published.index else "dashboard/explore.py"
