...
When one flat sells for more than another, how much of that is down to the flat itself
(its location, its remaining lease, its floor) and how much is down to *which* flats
happened to sell? I kept coming back to that question, and answering it meant collecting
the data, cleaning it, comparing flats like for like, and checking the answers held up.
...
- Living near an MRT station is worth about 19% more per square metre. A simple
  comparison puts it at 10%, because flats far from stations often have other things
  going for them, like a better town or a newer lease.
  ([Finding 2](FINDINGS.md#2-living-near-an-mrt-station-is-worth-more-than-it-first-looks))
- Older flats looked *more* expensive at first, because the oldest flats are in central,
  mature estates. Comparing like with like, a flat with under 50 years of lease left is
  worth about 43% less than one with 90+ years.
  ([Finding 4](FINDINGS.md#4-older-flats-looked-more-expensive-until-i-compared-like-with-like))
- A high floor is worth about 20%. A simple comparison shows 83%, but tall blocks tend
  to be newer and in pricier areas.
...
- One of my ideas turned out to be wrong, and I kept it in: buyers in different regions
  did not move to bigger or smaller flats in different ways.
...
How the data flows. Each box is one step in dbt, from the raw data on the left to the
final published tables and checks on the right.
...
building the database, and checking that the write-up matches the published numbers.
...
[The dashboard is live here](https://hdb-resale-pipeline-swbi9a9mem46f9g87msje4.streamlit.app/), with nothing to install.
...
- Distance to popular primary schools. Living within 1km of one matters for Primary One
  registration, so it's probably the biggest price factor I'm missing.
- A test of forecasting: how badly a simple forecast would have done in past years,
  which would show why I don't make predictions here.
